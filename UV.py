...
def mainPage():
    ...
    with st.form("my_form"):
        ...
        if submitted:
            ...
            strCol = pd.json_normalize(pd.json_normalize(placeData['results'][0])['photos'].iloc[1:20].str[0] ).photo_reference.apply(lambda x: 'https://maps.googleapis.com/maps/api/place/photo?photo_reference=' + str(x) + '&key=' + st.secrets["weather_key"])
